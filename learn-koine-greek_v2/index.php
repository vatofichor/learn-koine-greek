...
<head>
    ...
    <title>Koine Greek Course Viewer</title>
...
<body>
    ...
    <div class="window">
        ...
        <div class="window-header">
            ...
            <div class="window-title">Koine Greek Course Viewer</div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadAndGoToArticle(order) {
                ...
                try {
                    ...
                    loadedSubdirs.forEach((container) => {
                    ...
                    if (!loadedSubdirs.has(subdir)) {
                        ...
                        // Fetch and render all articles inside this subdir container
                        for (let i = 0; i < subdirArticles.length; i++) {
                            const art = subdirArticles[i];
                            try {
                                ...
                                // Save references for scrollspy checks
                        ...
                        // Add Next Section button at the end of the subdir container if there is a next article
                        const lastArt = subdirArticles[subdirArticles.length - 1];
                        ...
                        loadedSubdirs.set(subdir, container);
                    }
            ...
            // Resolves relative asset paths from article subfolder to workspace root
            ...
            function renderArticle(rawContent, relativePath) {
                ...
                // If the file path indicates it was already compiled HTML, output directly
                if (relativePath.includes('Alphabet-and-Counting')) {
                    return stripped;
                }

                // Otherwise run markdown parser
            ...
            function parseMarkdown(md, relativePath) {
                ...
                // Escape simple tags but keep math tags clean
                html = html
                    .replace(/&/g, '&amp;')
                    .replace(/</g, '&lt;')
                    .replace(/>/g, '&gt;');
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (line.startsWith('|')) {
                        ...
                        if (inTable) {
                            ...
                            newLines.push(`<!--TABLEBLOCK_${tableBlocks.length - 1}-->`);
                            tableRows = [];
                            inTable = false;
                        }
                ...
                // 3. Headings (supporting leading whitespace/indentation)
                ...
                // 4. Blockquotes (supporting leading whitespace/indentation)
                html = html.replace(/^\s*&gt;\s*(.*)$/gm, '<blockquote>$1</blockquote>');
                html = html.replace(/<\/blockquote>\n<blockquote>/g, '\n');

                // 5. Horizontal rules (supporting leading whitespace/indentation)
                ...
                // 8. Bold / Italic / Images / Links / Code inline
                html = html.replace(/\*\*([^\*]+)\*\*/g, '<strong>$1</strong>');
                html = html.replace(/\*([^\*]+)\*/g, '<em>$1</em>');
                html = html.replace(/_([^_]+)_/g, '<em>$1</em>');
                html = html.replace(/`([^`]+)`/g, '<code>$1</code>');

                // 8b. Markdown Images with relative URL resolution (must run before link replacement)
                html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, src) => {
                    const resolvedSrc = resolveUrl(src, relativePath);
                ...
                // 8c. Markdown Links (must run after images)
                ...
                for (let i = 0; i < paragraphs.length; i++) {
                    ...
                    if (p.startsWith('<h') || p.startsWith('<blockquote') || p.startsWith('<ul') || p.startsWith('<hr') || p.startsWith('<!--') || p.startsWith('<table')) {
                        continue;
                    }
                ...
                // 10. Restore code blocks and tables
                codeBlocks.forEach((block, index) => {
                    html = html.replace(`<!--CODEBLOCK_${index}-->`, block);
                });
                tableBlocks.forEach((block, index) => {
                    html = html.replace(`<!--TABLEBLOCK_${index}-->`, block);
                });

                // Restore special math symbol entities in final html output
                html = html
                    .replace(/&amp;(\w+);/g, '&$1;') // restore simple entities like &alpha;
                    ...
                    .replace(/\\text\{([^\}]+)\}/g, '$1'); // clean up simple math text macros
