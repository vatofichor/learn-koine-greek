...
<body>
    ...
    <div class="window">
        ...
        <div class="window-header">
            ...
            <div class="window-title">Learn Greek</div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function appendNextSectionCard(container, lastArt) {
                ...
                if (nextArt) {
                    ...
                    const nextFilename = nextArt.relative_path.split('/').pop().replace(/\.html$/i, '');
                    const nextCleanFilename = nextFilename.replace(/^\d+[-_]/, '').replace(/[-_]/g, ' ');
                    
                    nextCard.innerHTML = `
                        <div class="next-section-info">
                            <div class="next-section-label">Next Section</div>
                            <div class="next-section-title">${nextArt.title || nextCleanFilename}</div>
            ...
            function updateTOCAndProgress(order) {
                ...
                if (courseManifest[order]) {
                    ...
                    const cleanFilename = filename.replace(/^\d+[-_]/, '').replace(/[-_]/g, ' ');
                    statusLeft.textContent = `Reading: ${cleanFilename} (${cleanSectionName(getSubdir(courseManifest[order].relative_path))})`;
