...
### Security Enforcement & Validation Rules
- **Protocol Restrictions**: Link and image URLs must be relative (e.g. starting with `/`, `.`, or containing no protocol colon scheme) or use explicit standard web protocols (`http://` or `https://`). All other schemes (such as `javascript:`, `data:`, `vbscript:`, or mailto targets) are blocked and compiled to a safe `#` fallback anchor.
- **HTML Quote Escaping**: Double quotes `"` and single quotes `'` inside URLs or image alt text are automatically transformed into standard safe HTML entities (`&quot;`, `&#39;`) to prevent HTML attribute breakouts and client-side execution attacks.
