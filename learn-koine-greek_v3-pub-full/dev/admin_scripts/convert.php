...
foreach ($mdFiles as $mdFile) {
    ...
    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
        ...
        $cmd = 'cscript /Nologo ' . escapeshellarg($jsCompiler) . ' ' . escapeshellarg($tempMdFile) . ' ' . escapeshellarg($htmlOutputFile) . ' ' . escapeshellarg('content/' . $htmlRelPath);
        $output = [];
        $returnVal = 0;
        ...
        if ($returnVal === 0) {
            ...
            // Try node
            $cmd = 'node ' . escapeshellarg($jsCompiler) . ' ' . escapeshellarg($tempMdFile) . ' ' . escapeshellarg($htmlOutputFile) . ' ' . escapeshellarg('content/' . $htmlRelPath);
            exec($cmd, $output, $returnVal);
            if ($returnVal === 0) {
                $compiled = true;
            }
        ...
        // Unix/macOS: try node
        $cmd = 'node ' . escapeshellarg($jsCompiler) . ' ' . escapeshellarg($tempMdFile) . ' ' . escapeshellarg($htmlOutputFile) . ' ' . escapeshellarg('content/' . $htmlRelPath) . ' 2>&1';
        $output = [];
        $returnVal = 0;
        exec($cmd, $output, $returnVal);
        if ($returnVal === 0) {
